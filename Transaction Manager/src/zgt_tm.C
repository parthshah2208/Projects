...
int zgt_tm::TxRead(long tid, long obno, int thrNum)
 {   
   ...
   fflush(stdout);
#endif
   pthread_t thread1;                   //using pthread create a thread
   struct param *nodeinfo = (struct param*)malloc(sizeof(struct param));  //dynamically allocate memory to a node
   nodeinfo->tid = tid;                             //set tid, obno, type of transaction as 'R'
   nodeinfo->obno = obno;
   nodeinfo->Txtype = 'R';
   nodeinfo->count = --SEQNUM[tid];                 //Count is reduced by one because its a read operation
   int status;
   status = pthread_create(&threadid[thrNum],NULL,readtx,(void*)nodeinfo);//create a pthread and call the readtx() function with the node information we have given and threadid
   if (status) //if creating thread fails then display error.
   {
     printf("ERROR: return code from pthread_create() is:%d\n", status);
     exit(-1);
   }   
...
int zgt_tm::TxWrite(long tid, long obno, int thrNum)
 {
   ...
   struct param *nodeinfo = (struct param*)malloc(sizeof(struct param));        //dynamically allocate memory to a node
   nodeinfo->tid = tid;                             //set tid, obno, type of transaction as 'W'
   nodeinfo->obno = obno;
   nodeinfo->Txtype = 'W';
   nodeinfo->count = --SEQNUM[tid];                 //Count is increased by one because its a write operation
   int status;
   status = pthread_create(&threadid[thrNum],NULL,writetx,(void*)nodeinfo);//create a pthread and call the wrtietx() function with the node information we have given and threadid
   if (status)
   {
     printf("ERROR: return code from pthread_create() is:%d\n", status);  //if creating thread fails then display error.
     exit(-1);
   }
#ifdef TM_DEBUG
   printf("\nleaving TxWrite\n");
...
int zgt_tm::CommitTx(long tid, int thrNum)
 {
   pthread_t thread1;                                   //using pthread create a thread
   struct param *nodeinfo = (struct param*)malloc(sizeof(struct param));        //dynamically allocate memory to a node
   nodeinfo->tid = tid;                            //set tid, obno, type of transaction as '' because it is a commit function , obno=-1 because we are commenting a TX
   nodeinfo->Txtype = ' ';
   nodeinfo->obno = -1;
   nodeinfo->Txtype = ' ';
   nodeinfo->count= --SEQNUM[tid];
   int status;
   status = pthread_create(&threadid[thrNum], NULL, committx,(void*)nodeinfo);//create a pthread and call the committx() function with the node information we have given and threadid
   if (status)
   {
     printf("ERROR: return code from pthread_create() is:%d\n", status);//if creating thread fails then display error.
     exit(-1);
...
int zgt_tm::AbortTx(long tid, int thrNum)
 {
   pthread_t thread1;                                  //using pthread create a thread
   struct param *nodeinfo = (struct param*)malloc(sizeof(struct param));       //dynamically allocate memory to a node
   nodeinfo->tid = tid;                            //set tid, obno, type of transaction as '' because it is a abort function , obno=-1 because we are aborting a TX
   nodeinfo->obno = -1;
   nodeinfo->Txtype = ' ';
   nodeinfo->count = --SEQNUM[tid];
   int  status;
   status=pthread_create(&threadid[thrNum], NULL,aborttx,(void*)nodeinfo);		//create a pthread and call the aborttx() function with the node information we have given and threadid
   if (status)
   {
     printf("ERROR: return code from pthread_create() is:%d\n", status);        //if creating thread fails then display error.
     exit(-1);
