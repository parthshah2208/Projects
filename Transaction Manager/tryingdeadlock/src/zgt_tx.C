...
void *readtx(void *arg)
{
    ...
    zgt_tx *txnpntr = get_tx(tid);
    long sgnum =txnpntr->sgno;
    
    if(txnpntr !=NULL)
    {
        zgt_p(0);
        txnpntr->set_lock(tid,sgnum,objno,count,'S');
        zgt_v(0);
    }
...
void *writetx(void *arg)
{
    ...
    {
        zgt_p(0);
        txnpntr->set_lock(tid,sgnum,objno,count,'X');
        zgt_v(0);
    }
...
void *do_commit_abort(long t, char status)
{
    ...
    zgt_tx *txnpntr = get_tx(t);
    
    if(txnpntr!=NULL)
    {
        txnpntr->status = status;
        txnpntr->free_locks();
        txnpntr->remove_tx();
        
        int noWaiting = zgt_nwait(txnpntr->semno);
        
        for (int i = 1; i <= noWaiting; i++)
            zgt_v(t);
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  //write your code
    zgt_hlink *currentOwner, *newOwner;
    open_logfile_for_append();
    ...
    if(currentOwner==NULL)
    {
        ...
        currentOwner = ZGT_Ht->find(sgno1, obno1);
        if(currentOwner==NULL)
        {
            ZGT_Ht->add(this, sgno1, obno1, lockmode1);
            perform_readWrite(tid1,sgno1,obno1);
            return(0);
        }
//        else
//        {
